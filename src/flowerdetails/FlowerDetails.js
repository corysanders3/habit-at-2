...
function FlowerDetails({ details, getDetails, loadForm, questionActive, setQuestionActive }) {
    ...
    return (
        <div className="absolute top-24 ml-10 min-w-fit w-1/5 rounded-xl bg-white text-lg leading-6 shadow-lg">
            ...
            <button 
                className="font-semibold mt-0 mb-5 mx-6 text-gray-60 bg-gray-200 p-3 pl-5 pr-5 rounded-md hover:bg-lime-200 hover:cursor-pointer"
                onClick={e => {loadForm(e, questionActive, setQuestionActive)}}>
                Got A Question About This Habit?
            </button>
        </div>
    )
}
